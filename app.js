...
app.get("/artist/", async function (req, res) {
	...
	try {
		...
		res
			.status(200) // response if endpoint is successful
			.json({
				status: "success",
				data: allArtists,
			});
	...
	try {
		...
		if (!data || !data.name) {
			res.status(400).json({
				...
				data: {
					message:
						"Please provide artist details in JSON format  with 'name property",
				},
			}); // handle the error if wrong id is provided
			return;
		}
...
app.delete("/artist/:id", async function (req, res) {
	...
	try {
		const artist = await deleteArtistById(id); //fetch the artist details using the provided ID
